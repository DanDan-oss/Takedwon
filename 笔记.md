...
'volatile 关键字'
    const int a = 20;	// 函数内的局部变量,之后使用变量的地方都会在编译阶段直接替换为值
    std::cout << a <<std::endl; // 编译阶段会被编译器替换为 std::cout << 20 <<std::endl  
    volatile const int a = 20;	// 函数内的局部变量,禁用编译器优化. 之后使用a的地方都会直接使用到空间
```
...
```c++
# 引用:在汇编中看,引用变量是有在栈空间中的,但是编译器做了语法糖,在语言中对引用变量取地址和打印,其实是获取的原始变量的地址
...
# 对数组进行引用
	...
	# 1
	typedef int(arrayfrce)[6];	// 定义数组类型
	arrayfrce& a = arr;

	# 2
	typedef int(&arrayfrce)[6];	// 定义引用数组类型
	arrayfrce a = aar;

	# 3  
	int(&a)[6] = aar;		// 直接定义引用类型

	# 4 
	auto& a = aar;		// 自动推导类型
       
# 不能直接对字面量(数值)进行引用 但是可以加 const对字面量进行引用
	int &ret = 10;	// 错误
	const int &ret=10;  //正确, 编译器会做int tmp=10; const int& ret=tmp;
...
connect(Object *sender, Func1 signal, QObject *context, Func2 slot);
	...
	// 方式二,函数带参数
	connect(&this->m_button_ok, &QToolButton::clicked, this, [this]() {
    	this->on_button_ok_clicked(42);  // 这里的 42 是你要传递的参数
	});
	// 方式三,函数带参数
	connect(&this->m_button_ok, &QToolButton::clicked, this, std::bind(&ServerConfig::on_button_ok_clicked, this, 42));
...
// 类函数
	...
	CString::GetBufferSetLength(int nLength); //返回指向缓冲区的指针,该缓冲区可用于直接修改,缓存区长度nLength
	CString::ReleaseBuffer(); 	/* GetBuffer()获得Buffer缓冲区使用后释放缓冲区的使用*/
	CString::Right(int num); 	/* 取字符右边N个字符*/
	CString::ReverseFind(char ch)const; /*字符串从后往前查找字符*/	
...
'WinDbg调试命令'
	...
	.reload			// 加载符号信息，以确保能够正确解析符号
	a 				// 进入修改指令界面
	!process 0 0	// 查看所有进程进程信息
			!process 0 0 <FirstDriverBaseAddress>	// 查看所有某个进程或者驱动的进程信息
	.process id 		// 使用某个进程的上下文
        .process /p 1234
        
	lm m *			// 查看当前系统加载的驱动模块列表
        	lm m <FirstDriverName>			// 查看系统加载的某个驱动信息
      

        
	!object \BaseNamedObjects // 查看驱动\\BaseNamedObjects 目录下的所有对象及其相关信息
```
...
![内存地址](image/Snipaste_2024-02-03_22-13-29.png)
...
/* 注册服务([sc create]:创建一个服务 [binPath]:驱动文件路径 [type]:驱动类型 [start]:该服务的启动类型) 
	注册表位置: HKEYLOCALMACHINE＼SYSTEM＼Cu∏entControlSet＼Services
*/
...
```cpp
/* 如果一个IRP的主功能号是'IRP_MJ_READ',它的次功能号,则应该有以下几种情况:
	IRP_MN_NORMAL: 由可能在Irp->MdlAddress中返回数据,也有可能在Irp->UserBuffer中返回数据.如果要完成这样的请求,直接判断Irp->MdlAddress是否为空即可.如果为空,则把数据填入Irp->UserBuffer中,这些空间都是预先分配好的,不需要编程者自己分配
	IRP_MN_MDL: 功能号为IRPMNMDL是比较罕见的.是自己分配一个MDL,然后令MDL指向数据所在的空间,最后返回给上层.MDL自然是要释放的,所以又有IRP_MN_MDL_COMPLETE,意思是一个MDL已经使用完毕,可以释放了.
	IRP_MN_MDL_COMPLETE: 可以释放MDL指向数据空间
/*
```
...
'IRP栈空间操作'
	...
	// 跳过当前设备栈设备,把请求发送给下层设备，其实就是使IO栈指针加1
...
```cpp
'中断级别'
    Passive				// 一般由3环程序的调用引发的中断
    Dispatch			// 一般由下层硬件引发的中断
```
...
// 驱动程序入口函数(驱动对象的指针, 结构指向当前驱动所对应的注册表位置)
	...
	PAGED_CODE();						// 此宏可以确保调用线程运行在一个允许分页的足够低IRQL级别
	PDRIVER_OBJECT PsGetCurrentDriver();	// 获取当前驱动对象
	PDRIVER_OBJECT FltGetDriverObject();	// 获取当前驱动对象, #include <fltKernel.h>
	PUNICODE_STRING pRegistryPath= &pDriverObject->DriverExtension->ServiceKeyName; // 通过驱动对象获取当前注册表路径
...
/* 格式化字符串:sprintf() */
	NTSTRSAFEDDI RtlStringCchPrintfA(NTSTRSAFE_PSTR pszDest, // 需要格式化到哪个字符数组里面(NTSTRSAFE_PSTR==char)
                                     ...
                                     ...); 						// 需要格式化的参数, N
	NTSTRSAFEDDI RtlStringCchPrintfW();				// 与上面用法一样
	NTSTATUS RtlAppendUnicodeToString(PUNICODE_STRING, PCWSTR);	// 讲UNICODE_STRING字符串拷贝添加到PCWSTR
...
```cpp
void ExInitializeFastMutex(PFAST_MUTEX FastMutex);		// 始化一个快速互斥变量，用于同步一组线程对共享资源的互斥访问
```
...
>
> `\\BaseNamedObjects` 是一个特殊的内核对象目录，它存储了许多命名对象。在这个目录下，您可以创建和访问具有唯一名称的内核对象
...
typedef enum _ObjectType	/*指向对象类型的指针*/
{
    ...
    *TmTransactionObjectType
    ...
    VOID InitializeObjectAttributes(POBJECT_ATTRIBUTES p, /* 需要初始化的结构*/
                                    PUNICODE_STRING n, /* 打开谁的句柄(对象的名字)*/
                                    ULONG a, /* 标志/权限* OBJ_KERNEL_HANDLE | OBJ_CASE_INSENSITIVE */ 
                                    HANDLE r, /* 可以为NULL*/
...
'句柄对象'	/* <ntifs.h> P52*/
	//创建一个句柄对象
	NTSYSAPI ZwCreateEvent(PHANDLE EventHandle, /* 用于保存EVENT句柄*/
                                    ACCESS_MASK DesiredAccess, /* EVENT打开权限*/
                                    POBJECT_ATTRIBUTES ObjectAttributes, /* EVENT属性结构*/
                                    EVENT_TYPE EventType, /* EVENT类型，SynchronizationEvent||NotificationEvent*/
                                    BOOLEAN InitialState); /* EVENT的初始状态,默认FALSE*/
	// 打开一个句柄对象,参数类型与ZwCreateEvent()相同
...
// 获取句柄对象(EventHandle)对应的Event对象指针 ---对象化 <<Windows内核编程•P53>> 
    ...
    NTSTATUS ObReferenceObjectByHandle(HANDLE Handle, 	/* ZwCreateEvent打开的Event对象的句柄*/
                                       ...
                                       POBJECT_TYPE ObjectType, 	/* 对象的类型看上面定义,*ExEventObjectType||... */
                                       KPROCESSOR_MODE AccessMode, 	/*访问模式(用户态和内核态)，UserMode||UserMode */
...
}IO_STATUS_BLOCK, *PIO_STATUS_BLOCK; // 函数操作结果结构 <<Windows内核编程•P61>>

struct LARGE_INTEGER; 	// 文件读取位置结构体(读取起始位置)

typedef enum _ACCESS_MASK {
    FILE_WRITE_DATA, FILE_READ_DATA, FILE_DELETE_DATA, FILE_WRITE_ATTRIBUITES, FILE_READ_ATTRIBUITES,
    GENERIC_READ, FENERIC_WRITE, GENERIC_ALL, SYNCHRONIZE, ...
}ACCESS_MASK;
...
typedef struct _OBJECT_NAME_INFORMATION{
	...
	NTSTATUS ZwCreateFile(OUT PHANDLE FileHandle, 	/* 文件打开后,句柄存放的地址*/
                          ...
                          IN ULONG FileAttributes, 	/* 控制新建立的文件属性,一般为FILE_ATTRIBUTE_NORMAL或0*/
                          IN ULONG ShareAccess, 	/* 共享访问权限,允许别的代码打开这个文件所持有的权限*/
                          IN ULONG CreateDisposition, 	/* 文件打开意图(新建/打开/覆盖...),选择项不能组合*/
                          IN ULONG CreateOptions, 	/* 同步操作(防止异步操作产生未决),一般默认三个参数FILE_NON_DIRECTORY_FILE | FILE_SYNCHRONOUS_IO_NONALERT|FILE_RANDOM_ACCESS */
	...
	// 获取调用方具有指针的给定对象的名称
	NTSTATUS ObQueryNameString(PVOID Object, POBJECT_NAME_INFORMATION ObjectNameInfo, ULONG Length,
                              PULONG ReturnLength);
```
...
```cpp
	...
	/*线程的关闭,应该线程自己调用PsTerminateSystemThread()结束*/
	...
	ZwClose(HANDLE);		// 关闭线程句柄,只会关闭对线程的引用,并不会杀死线程
...
```cpp
// ExInitializeWorkItem 使用调用方提供的上下文和回调例程初始化工作队列项，以便在系统工作线程获得控制权时排队等待执行。
...
```cpp
	...
	NTSTATUS IoCreateDeviceSecure(IN PDRIVER_OBJECT DriverObject, /* 驱动对象,直接从入口函数获取(DriverEntry)*/
                                  ...
                                  IN DEVICE_TYPE DeviceType, /* 设备类型,FILE_DEVICE_UNKNOWN*/
                                  IN ULONG DeviceCharacteristics, /* 设备属性,FILE_DEVICE_SECURE_OPEN*/
	...
	NTSTAUS IoCreateDevice(IN PDRIVER_OBJECT DriverObject, 	/* 驱动对象,直接从入口函数获取(DriverEntry)*/
                           IN ULONG DeviceExtensionSize, /* 设备扩展的大小,默认写0就行*/
                           ...
                           OUT PDEVICE_OBJECT *DeviceObject);
								/* 生成的设备对象指针(传入一个保存DEVICE_OBJECT句柄的地址)*/
...
'设备绑定'
    ...
    NTSTATUS IoAttachDevice(IN PDEVICE_OBJECT SourceDevice, 	/* 生成的过滤设备*/
                            IN PUNICODE_STRING TargetDevice, 	/* 被绑定的目标设备*/
...
```cpp
	...
	NTSTATUS WdmlibIoCreateDeviceSecure(PDRIVER_OBJECT DriverObject, ULONG DeviceExtensionSize,PUNICODE_STRING DeviceName,  DEVICE_TYPE DeviceType, ULONG DeviceCharacteristics, BOOLEAN Exclusive, PCUNICODE_STRING DefaultSDDLString, LPCGUID DeviceClassGuid, PDEVICE_OBJECT* DeviceObject);
...
'获取R3层数据交换发送虚拟地址 <<Windows内核编程 •P114>>' 
	...
	MmGetSystemAddressForMdlSafe(PMDL Mdl, ULONG Priority);			// Windows高版本使用
		// PBYTE* pBuf = MmGetSystemAddressForMdlsafe(irp->MalAddress, NorMalPagePriority); 参数2是隐射属性
...
/* 设置一个设备对象名称和该设备的用户可是名称之间的字符链接*/
	...
	// 符号链接建立,一定要设置驱动的处理回调函数; DriverObject->MajorFunction[IRP_MJ_CREATE] = MyDeviceCreate;
...
```cpp
	...
	IoGetDeviceObjectPointer();		// 通过名字获取设备对象
...
> KbdClass被称为'键盘类驱动'在Windows中,类驱动通常是指统管一类设备的驱动程序.不管是USB键盘,还是PS／2键盘均经过它,所以在这层做拦截,能获得很好的通用性.在类驱动之下和实际埂件交互的驱动被称为"端口驱动". 具体到键盘,j8042prt是PS/2键盘的端口驱动,USB键盘则是Kbdhid

> 键盘过滤可以通过`往设备栈中添加新设备(添加设备栈方式,容易被检测, 遍历设备栈发现陌生设备)`、`HOOK键盘驱动对象(Kbdclass)的分发函数指针过滤`、`HOOK i8042prt端口驱动的I8042KeyboardInterruptService中类驱动的回调函数指针`
...
NTSTATUS ObReferenceObjectByName(PUNICODE_STRING ObjectName, ULONG Attribuites, PACCESS_STATE AccessState,
...
##### 6.3.2.4.3  HOOK端口驱动函数中的类驱动回调函数

> 在i8042prt的自定义设备扩展中,保存着一些指针和计数值，用来使用它的输入数据队列.包括:
> (1)PKEYBOARDINPUTDATA类型的InputData、DataIn、DataOut、DataEnd.
> (2)ULONG类型的InputCount.
>     InputData指针,指向输入数据队列的开头.
>     DataEnd指针,指问输入数据队列的结尾.
...
```cpp
/* <<Windows内核编程>> P154
当键盘上—个键被按下时,产生—个MakeCode引发键盘中断;
当键盘上—个键被松开时,产生一个BreakCode引发键盘中断;
键盘中断导致键盘中断服务例程被执行,最终导致i8042prt的I8042KeyboardInterruptService被执行.
在I8042KeyboardInterruptService中,从端口读出按键的扫描码,放在一个KEYBOARD_INPUT_DATA中.
将这个KEYBOARD_INPUT_DATA放入i8042ptr的输入数据队列中,一个中断放入一个数据,
DataIn后移一格,InputCount加l.最后会调用内核API函数KeInsertQueueDpc.

在这个调用中，会调用上层处理输入的回调函数(也就是KbdClass处理输入数据的函数),
取走i8042ptr的输入数据队列里的数据.因为设备扩展中保存着上层处理输入数据的回调函数的
入口地址,所以它知道该调用谁.上层处理输入的回调函数(也就是KbdClass处理输入数据的
函数)取走数据之后, i8042ptr的输入数据队列的DataOut相应后移,InputCount相应减少

从上面的原理来看I8042KeyboardInterruptService中调用的类驱动的回调函数非常关键.
如果找到了这个函数,通过Hook、替换或者类似的手段,就可以轻易地获得键盘的输入了.
而且这个函数非常深入,也并没有公开,安全软件很难顾及到.
现在的问题就是如何定位这个函数指针了,i8042prt驱动的设备扩展我们并不完全清楚;
此外,WDK中也不可能公开这样—个函数的地址.但是"有识之士"根据经验指出:
（l）这个函数指针应该保存在l8042prt生成的设备的自定义设备扩展中/
（2）这个函数的开始地址应该在内核模块KbdClass中.
（3）内核模块KbdClass生成的一个设备对象的指针也保存在那个设备扩展中,而且在我们要找的函数指针之前.
*/

// 通过下面的简单代码就可以判断一个地址是否在KbdClass驱动中
PVOID address;
size_t KdbDriverStart = KdbDirverObject->DeviceStart;
size_t KdbDriverSize = KdbDriverObject->DriverSize;
...
if(address > KdbDriverStart && address < (PBYTE)KdbDriverStart+KdbDriverSize)
{
    // 说明地址在驱动中
}
...
/* 搜索类驱动生成的设备对象,这个设备对象的指针保存在端口驱动的设备对象的扩展中,
而且必须先找到它,后面才能搜索回调函数(根据前面指出的搜索时用的三个规津).
*/
...
{
    PDEVICE_OBJECT classDeviceObject;	// 类驱动生成的设备对象
    KeyBoardClassServiceCallBack ServiceCallBack;    // 搜索的回调函数
...
//初始化驱动程序 的WDF_DRIVER_CONFIG 结构,并设置设备添加回调函数
void WDF_DRIVER_CONFIG_INIT(PWDF_DRIVER_CONFIG Config, PFN_WDF_DRIVER_DEVICE_ADD EvtDriverDeviceAdd);

/* 方法将设备名称分配给设备的设备对象*/
NTSTATUS WdfDeviceInitAssignName(NTSTATUS WdfDeviceInitAssignName, NTSTATUS WdfDeviceInitAssignName);

// 设置指定设备的设备类型,所有的磁盘设备类型都要设置为FILE_DEVICE_DISK
void WdfDeviceInitSetDeviceType(PWDFDEVICE_INIT DeviceInit, DEVICE_TYPE DeviceType);

// 设置驱动程序如何访问指定设备的读取和写入请求中包含的数据缓冲区的方法或首选项
void WdfDeviceInitSetIoType(PWDFDEVICE_INIT DeviceInit, WDF_DEVICE_IO_TYPE IoType);
...
```cpp
/* 宏函数, 递增或递减调用方提供的页文件计数器作为原子操作*/
IoAdjustPagingPathCount(_count_, _paging_)
```
...
> 在Windows上,熟知的文件系统是FAT32和NTFS,这个两个驱动都在Windows系统目录的driver目录下.  FAT32的驱动在Windows下是`fastfat.sys`,NTFS驱动在Windows下叫`ntfs.sys`, 
>
> Wmdows的标准文件系统识别器似乎都是由驱动”`\\FileSystem\\Fs_Rec`”生成的,要注意没有人规定文件系统识别器一定在驱动`\\FileSystem\\Fs_Rec`”下生成
>
> 文件系统一般生成两类设备, 
>
> ​	1): 一种是控制设备(CDO),主要任务是修改整个驱动的内部配置,所有一个文件系统只对应一个CDO.
>
> ​	2): 一种是文件系统的卷设备,一般一个卷对应一个逻辑盘.  实际“C:”是设备的符号链接(SymbolicLink)名,而不是真正的设备名,“C:”这个符号链接名对应的设备名是"\Device\HarddiskVolume1",因此该设备的真名为 "\Device\HarddiskVolume1”. 这个卷设备本身并不是文件系统驱动生成的,而是卷管理器生成的. 但是当有—个卷使用了某种文件系统时,该文件系统会对应地为该设备生成一个没有名字的设备对象(文件系统的卷设备)
>

```cpp
/* 快速IO分发函数*/
/*
快速IO分发函数是独立于普通的处理IRP的分发函数之外的另一组接口.但是它们的作用与IRP分发函数是
一样样的,就是由驱动处理外部给予的请求.而且所处理的请求也基本相同,只是根本没有IRP,本
来应该写在IRP中的参数,直接通过函数的参数被传递进来了.这可以减少分配IRP的效率消耗
可以简单地设置所有的快速lO分发函数返回FALSE并不做任何事，这样这些请求都会通过IRP重新发送而被普通IRP分发函数捕获.
大部分情况下性能会损耗10%, 极个别情况下会损耗更多
*/
typedef struct _FAST_IO_DISPATCH {
    ULONG SizeOfFastIoDispatch;
    
    // 此调用并不是由IO管理器直接调用的,而是被FsRtlXXX系列函数调用,用于确认读/写操作是否可以用快速IO接口进行
    ...
    // 用于提供NtDeviceIoControlFile的支持
    PFAST_IO_DEVICE_CONTROL FastIoDeviceControl;
    
    // NTFS在映射文件内容到内存页面前进行的操作
    PFAST_IO_ACQUIRE_FILE AcquireFileForNtCreateSection;
    PFAST_IO_RELEASE_FILE ReleaseFileForNtCreateSection;
    
    // 当可移动介质被拿走后,文件系统的设备对象会在任意时刻被销毁,
    // 只有正确地处理这个调用,才能使上层设备和将要销毁的设备解除绑定.
    // 如果不解决这个函数,系统会直接崩溃
    PFAST_IO_DETACH_DEVICE FastIoDetachDevice;
    
    // 当CIFS(网上邻居、网络重定向驱动,尝试获取文件信息时会调用)
    // 该调用是因为各种历史原因而产生的，当时设计CIFS是为了避免多次在网上传输文件信息请求，
    // 在NT4时传输协议增加了一个FileNetworkOpenInformation网络文件请求,而FSD则增加了这个接口,
    // 用于在一次操作中获得所有的文件信息;客户端发送FileNetworkOpenInformation,服务器端的FSD用本接口完成信息填写
    PFAST_IO_QUERY_NETWORK_OPEN_INFO FastIoQueryNetworkOpenInfo;
    
    //Modified Page Writer会调用这个接口来获取文件锁
    PFAST_IO_ACQUIRE_FOR_MOD_WRITE AcquireForModWrite;
    
    PFAST_IO_MDL_READ MdlRead;
    PFAST_IO_MDL_READ_COMPLETE MdlReadComplete;
    
    // FSD提供MDL,以后向此MDL写入数据就代表向文件写入数据.调用参数中有FILE_OBJECT用于描述要写的目标文件
    PFAST_IO_PREPARE_MDL_WRITE PrepareMdlWrite;
    // 写操作完成, FSD回收MDL
    PFAST_IO_MDL_WRITE_COMPLETE MdlWriteComplete;
    // 当此调用被调用时,读到的数据是压缩后的.应该兼容于标准的NT提供的压缩库,因为调用者负责解压缩
    PFAST_IO_READ_COMPRESSED FastIoReadCompressed;
    // 当此调用被调用时,可以将数据压缩后存储
    PFAST_IO_WRITE_COMPRESSED FastIoWriteCompressed;
    // MDL版本的压缩读写. 当后一个接口被调用时,MDL必须被释放
    PFAST_IO_MDL_READ_COMPLETE_COMPRESSED MdlReadCompleteCompressed;
    PFAST_IO_MDL_WRITE_COMPLETE_COMPRESSED MdlWriteCompleteCompressed;
    
    // 这不是打开文件的操作,但是却提供了一个IRP_MJ_CREATE的IRP。这个操作是打开文件、获取文件基本信息、关闭文件的操作
    PFAST_IO_QUERY_OPEN FastIoQueryOpen;
    
    // 释放FastIoAcquireForModWrite调用所占有的锁
    PFAST_IO_RELEASE_FOR_MOD_WRITE ReleaseForModWrite;
    
    // FsRtlXXX系列会调用此接口,在延迟写线程将要把修改后的文件数据写入前调用,获取文件锁
...
```cpp
// 动态地寻找一个内核函数的指针,SystemRoutjneName是一个字符串,是要动态寻找地址的内核函数的名字
...
# 指定需要依赖编译的源文件
file(GLOB SRC_FILE "./include/*.c")


#用例 $cat ./CMakeLists.txt
include_directories(./include)	#头文件
file(GLOB SRC_FILE "./include/*.c")
add_executable(dpi.out ./Main.c ${SRC_FILE})	#编译规则
...
```shell
# 相关依赖, openssl(加解密)、PCRE(解析正则表达式)、zlib(压缩解压缩)
...
./configure \
--prefix=/home/build_tools/nginx \
--pid-path=/var/run/nginx/nginx.pid \
--lock-path=/home/build_tools/nginx/logs/error.log \
--error-log-path=/home/build_tools/nginx/logs/error.log \
--http-log-path=/home/build_tools/nginx/logs/access.log \
--with-http_gzip_static_module \
--http-client-body-temp-path=/home/build_tools/nginx/client \
--http-proxy-temp-path=/home/build_tools/nginx/proxy \
--http-fastcgi-temp-path=/home/build_tools/nginx/fastcgi \
--http-uwsgi-temp-path=/home/build_tools/nginx/scgi \
--add-module=/home/build_tools/fastdfs-nginx-module/src
...
# 编译并安装
make && make install
chmod 755 -R /home/build_tools/nginx
ln -sf /home/build_tools/nginx /usr/local/nginx
...
```shell
# nginx 配置文件 /home/build_tools/nginx/nginx.conf
...
http {
    ...
    access_log /home/build_tools/nginx/logs/access.log;
    error_log /home/build_tools/nginx/logs/error.log;

    server {
        listen 80 default_server;		# ipv4监听端口
        listen [::]:80 default_server;	# ipv6监听端口被
        root /home/build_tools/nginx/html;		# 服务器根目录
        #server_name _;
     ...
     server {
        ...
        index index.html;				
        location / {				# 服务器根目录
            root /home/build_tools/nginx/html;
            index  index.html index.htm;	#支持的请求文件格式
        }
        location /fast/ {	# 结尾带/表示资源根目录
            root /home/FastDFS/storage/storage_path0/data/M00;
        }

        location /fast {	# 表示命令
            # 配置fastcgi模块
            root /home/build_tools/nginx/html;				# 服务器根目录
            include /home/build_tools/nginx/conf/fastcgi.conf;	# 里面都是环境变量
            include /home/build_tools/nginx/conf/fastcgi_params		# fastCGI配置文件,nginx自带的
            fastcgi_pass localhost:9090;			# 将数据发送给cgi服务器的地址
        }
        location /group1/M00 {
            root /home/FastDFS/storage/storage_path0/data/M00;
            ngx_fastdfs_module;
...
![](./image/Snipaste_2024-11-10_01-49-57.png)
...
```shell
# 安装自动化生成脚本工具 gcc make m4 autoconf automake libtool
...
# 配置Nginx,看上面Nginx配置文件中


#spawn-fcgi的使用,IP必须使用和nginx配置文件里转发的IP一样,不然收不到消息
$: spawn-fcgi -a ${IP} -p ${PORT} -f 编译出来的fastcgi的处理程序 
```
...
```shell
# 编译 
$: gcc main.c -o fastcgi.exe
### 如果执行的时候fastcgi.exe报错找不到libfcgi.so.0 在/etc/ld.so.conf里面添加so的路径###
$: ldd fastcgi.exe
$: vi /etc/ld.so.conf
    /usr/local/lib
$: sudo ldconfig
### 或者在/etc/profile中写入 export LD_LIBRARY_PATH=/path/to/libs:$LD_LIBRARY_PATH  ###
...
![](./image/Snipaste_2024-11-10_01-41-46.png)
...
> 仓库地址: https://github.com/happyfish100、 fastDFS主要用于分布式存储
>
...
sudo ./make.sh clean && ./make.sh && ./make.sh install

# 编译libserverframe
cd libserverframe && git checkout V1.2.3
sudo ./make.sh clean && ./make.sh && ./make.sh instal

# 或者直接下载解压fastdfs对应的压缩包
...
#修改存储节点Storage的配置文件,mod_fastdfs.conf和storage.conf一样配
...
/* 通过客户端文件client.conf初始化数据 */
/* 参数:
	- conf_filename: client.conf配置文件，编译fastdfs.git生成默认存放/etc/fdfs/client.conf
  返回值: 成功等于0
*/
void fdfs_client_destroy();			// 去初始化

// 连接分发器
// 注意 远端服务器未开启连接不上的情况下,不会报错连接失败.程序会直接崩溃  *** buffer overflow detected ***: terminated
ConnectionInfo* tracker_get_connection(); // 失败返回NULL
void tracker_close_connection_ex(ConnectionInfo *conn, const bool bForceClose);
/* 关闭连接 */
/* 参数:
	- conn: tracker_get_connection()函数连接返回连接的分发器信息结构体
	- bForceClose: 
*/
...
/* 获取存储节点 */
/* 参数:
	- pTrackerServer: 分发器信息结构体
	- pStorageServer: 传出参数,存储节点信息结构体
	- group_name: 传出参数,作用未知,调试值为0
	- store_path_index: 传出参数,作用未知,调试值为group1
  返回值: 成功等于0
*/
...
/* 参数:
	- pTrackerServer: 分发器信息结构体
	- pStorageServer: 存储节点信息结构体
	- store_path_index: 获取存储节点时的传出参数,
	- local_filename: 需要上传的文件
	- file_ext_name: 默认为NULL
	- meta_list: 默认为NULL
	- meta_count:  默认为0
	- group_name: 获取存储节点时获取的传出参数
	- file_id: 传出参数, 文件在存储节点下存放的路径
  返回值: 成功等于0
*/
```
...
> pcap-dev开发包: `$: sudo apt-get install libpcap-dev`
>
> 查看帮助`man pcap`
>
...
|  |                                           | |
| ------ | ------------------------------------------------------------ | ------ |
| 应用层 | HTTP、FTP、Telnet、SMTP(e-mail) | 应用层数据 |
| 表示层 | ASCII、SSL/TLS   | 解析翻译 |
| 会话层 | ADSP、RPC | 发送时报文打包。 接收时校验报文完整性 |
| 传输层 | ICMP、TCP和UDP | 把包上传或下发 |
| 网络层 | IP、ARP、RARP和IGMP | 发送时报文添加ip地址。接收时校验ip是不是给自己的,不是转发 |
| 链路层 | HDLC、PPP、SLIP-->（以太网,局域网） | 发送时查表ip转mac转成电信号。接受时将电信号整理成帧 |
| 物理层 |  S 232C、RS 449/422/423、V.24和X.21、X.21BIS                               | 定义好各种信号，线路和插口的格式，发送 |

![](./image/v2-1578921092d775e024345fa8a531a85e_720w.png)
...
```cpp
	...
	*/
	...
	// 捕获并处理数据包。
	/* 参数
		- cnt: 此函数在cnt个数据包被处理或出现错误时才返回，参数为负值时pcap_loop()函数将始终循环运行.					但读取超时不会返回,可以默认传参0
...
```cpp
// Mac头部,总长度14字节 /usr/include/net/ethernet.h
...
{
    uint8_t ether_dhost[ETH_ALEN];	// dstmac，目标mac地址
    uint8_t ether_shost[ETH_ALEN];	// srcmac，源mac地址
    uint16_t ether_type;	// eth_type,以太网类型 	IP(ETH_P_IP:0x0800),ARP(ETH_P_ARP:0x0806),RARP(ETH_P_RARP:0x8035),IPV6(ETH_P_IPV6:0x86DD) 
}__attribute__ ((__packed__));   //用于告诉编译器不要对这个结构体中的缝隙部分进行填充操作；
```

`以太网(设备驱动程序及接口网 )[ 目的地址6字节, 源地址6字节, 类型2字节, 上层数据,CRC4z字节]`
...
```cpp
// IP头部,总长度20字节 /usr/include/netinet/ip.h
// 常用ip协议宏定义 /usr/include/netinet/in.h
/* 不能直接使用一个大结构体把几个直接包含头结构,以太网头最后类型那两字节会被内存对齐成4字节
错误用法
struct aptest
{
...
struct iphdr
  {
    ...
    unsigned int version:4;//版本 IPV4，IPV6
    unsigned int ihl:4;  //IP头长度,一般20字节=值x4(字节)
#else
# error "Please fix <bits/endian.h>"
    ...
    uint8_t ttl;       //路由器的跳转数
    uint8_t protocol;  //传输层协议, ICMP(IPPROTO_ICMP:1)，TCP(IPPROTO_TCP:6)，UDP(IPPROTO_UDP:17)
...
```
**选项段一般没有使用,详细参考RFC.  数据段为上层报文,如TCP 报文、UDP报文.**

![](./image/ip1.png)

IP如果拆包。如果是第一片取值为0，第二片取值175，以此类推。 
...
```c++
// tcp头部,总长度20字节 /usr/include/netinet/tcp.h
...
# elif __BYTE_ORDER == __BIG_ENDIAN
	...
	uint16_t fin:1;
# endif
	uint16_t window;	// 16位滑动窗口大小
	uint16_t check;		// 16位TCP头校验和
...
```cpp
// udp头 /usr/include/netinet/udp.h
...
TFTP_USERNAME="tftp"	
TDTP_DIRECTORY="/var/lib/tftpboot"	# TFTP_Server服务目录
TFTP_ADDRESS=":69"	# 默认为0.0.0.0:69,表示所有IP都可以访问
TFTP_OPTIONS="--secure"
# 修改为
TFTP_USERNAME="tftp"
TDTP_DIRECTORY="/tmp/"
TFTP_ADDRESS=":69"
TFTP_OPTIONS="-l -s -c"	
	# -l: 以standalone/listen模式启动TFTP
	# -c: 可创建新文件,TFTP默认只允许覆盖原有文件而不能创建新文件
	# -s: 改变TFTP启动根目录后,添加-s客户端使用不需要再输入指定目录,使用配置目录
...
| opcode | operation                                          |
| ------ | -------------------------------------------------- |
| 01     | Read request(RRQ) 读取请求                         |
| 02     | Write request(WRQ) 写请求                          |
| 03     | Data (DATA) 数据传输                               |
| 04     | Acknowledgment (ACK) 回应包                        |
| 05     | Error (ERROR), 错误应答,如请求传输一个不存在的文件 |
...
------------------------------------------------
特征: 2字节Opcode + Filename(长度不等至少1) +1字节0 + Mode(长度不等至少4)+ 1字节0
	Opcode: 01 | 02
	Mode: "netascii" | "mail" | "octet"
...
| Opcode | Block # | Data |
----------------------------------
特征: 2字节Opcode + 2字节Block + Data(0-512字节)
	Opcode: 03
...
| Opcode | Block # |
---------------------
特征: 2字节Opcode+2字节Block
	Opcode: 4
	Block: 包号为要确认的数据包的包号,WRQ数据包被ACK数据包确认，WRQ数据包的包号为0
...
-----------------------------------------
特征: 2字节Opcode + 2字节ErrorCode + ErrMsg(长度不等至少1) + 1字节0
	Opcode: 5
	ErrorCode: 0(未定义) | 1(文件未找到) | 2(访问非法) | 3(磁盘满或超过分配的配额) | 
		4(非法的TFTP操作) | 5(未知的传输ID) | 6(文件已经存在) | 7(没有类似的用户)
```
...
# 对时
$: sudo ntpdate -u ntp.api.bz	# 同步中国(国家授时中心)服务器时间210.72.145.44
```
...
# if __BYTE_ORDER == __LITTLE_ENDIAN
    ...
    uint8_t li:2;
# elif __BYTE_ORDER == __BIG_ENDIAN
    uint8_t li:2;	// 长度2个位. 值为“11”时表示告警状态时钟未被同步,为其他值时NTP本身不做处理
	uint8_t vn:3;	// 长度为3个位. 表示NTP的版本号.目前的最新版本为4
	uint8_t mode:3;	// 长度为3个位，表示NTP的工作模式。不同的值所表示的含义分别是：
    					/*0未定义、1表示主动对等体模式、2表示被动对等体模式、3表示客户模式、4表示服务器模式、5表示广播模式或组播模式、6表示此报文为NTP控制报文、7预留给内部使用 */
# endif
    uint8_t stratum;	    // 系统时钟的层数，取值范围为1～16，它定义了时钟的准确度。层数为1的时钟准确度最高，准确度从1到16依次递减，层数为16的时钟处于未同步状态，不能作为参考时钟,0表示未填充
    uint8_t poll;		// 轮询时间, 即两个连续NTP报文之间的时间间隔。
    uint8_t precision;	// 系统时钟的精度
    uint32_t root_delay;	//本地到主参考时钟源的往返时间
    uint32_t root_dispersion;	// 系统时钟相对于主参考时钟的最大误差
    uint32_t reference_identifier;	//参考时钟源的标识
    uint64_t reference_timestamp;	// 系统时钟最后一次被设定或更新的时间
    uint64_t originate_timestamp;	// NTP请求报文离开发送端时发送端的本地时间
    uint64_t receive_timestamp;		// NTP请求报文到达接收端时接收端的本地时间
    uint64_t transmit_timestamp;	// 应答报文离开应答者时应答者的本地时间
    uint96_t authenticator;			// 验证信息(可以没有)
}
```
...
sql> create user 'root'@'localhost' identified by '123456';
#允许外网IP访问，%通配所有远程主机
...
'Windwos' 安装mysql开发包: 
请注意下载的DLL和LIB是不是可调试的,发布版LIB和DLL只能使用Relese模式运行,DBUG模式会崩溃
```
...
#在原有的基础上增加新IP.如果想让所有的机器都能访问,就把这行注释掉,并将保护模式protected-mode 改成no
# 如果远程还是访问不了,检查一下防火墙
bind 127.0.0.1 192.168.88.128
protected-mode yes

# redis日志存放目录,默认/dev/null
logfile "/home/redis/redis.log"		

# 是否以守护进程的方式启动
daemonize no
pidfile /home/redis/redis.pid		# 设置以守护进程启动时pid文件存放位置

# redis默认数据库数量
databases 16

#客户端无操作断连时间,0表示超时无效,单位为秒
timeout 0

# redis持久化 rdb模式
rdbcompression yes	# 开启rdb存储模式
# 设置redis镜像数据库镜像备份频率
save 900 1		# 900秒之内有1个keys发生变化时
save 300 10		# 300秒之内有10个keys发生变化时
save 60 10000	# 60秒之内有10000个keys发生变化时
dbfilename redis.rdb	#数据库镜像备份文件名称
dir /var/lib/redis		# 数据库镜像备份文件路径

# redis持久化 Aof模式
appendonly no	# 开启Aof存储模式
...
#进入Redis
$: redis-cli
redis> keys *  # 查询当前所有的key
	redis> keys str* # 查询当前所有的str*的键
redis> RENAME key newkey  # 修改键的名字
```

```shell
redis> incr key 		# 将值进行自增1操作
redis> decr key 		# 将值进行自减1操作

redis> EXPIRE key seconds	# seconds秒后key自动删除
	redis> EXPIRE age 10
redis> TTL key			# 查看key剩余的生存时间
redis> PERSIST key		# 清除生存时间删除
```
...
redis> SET key value [NX | XX] [GET] [EX seconds | PX milliseconds | EXAT unix-time-seconds | 
	...
	redis> mset str1 123 str2 456 	# mset批量设置不能搭配EX使用
	
redis> GET key					# 查询
	redis>get age
	redis> mget str1 str2
	
redis> DEL key [key ...]		# 删除
	redis> del age

# List 队列、栈
# 遍历list,下标start是正数从前往后查,查询到下标stop, start是负数从后往前查,stop必须大于当前list下标大小
redis> LRANGE key start stop		
	redis> LRANGE itlist 0 -1	# 查询所有
	redis> LLEN itlist		# 查看有多少条数据
...
# Set 集合
redis> SMEMBERS key			# 查询集合中的元素
redis> SADD key member [member ...] 	# 向集合中添加元素, 元素是唯一的,重复的元素不会重复添加
redis> SREM key member [member ...] 	# 从集合中删除元素
redis> SDIFF key [key ...] 			# 获取多个集合中的差集
	redis> SDIFF A B C				# 获取别的集合中没有,集合A独有的元素. (A减B减C)
redis> SINTER key [key ...] 		# 获取多个集合中的交集(在所有集合中都存在的元素)
redis> SUNION key [key ...] 		# 获取多个集合中的并集(所有集合的所有元素,重复的元素只保留一个)

# SortSet 集合
# SortedSet会根据分数进行排序
redis> ZADD key [score member] ...
redis> ZRANGE key start stop [WITHSCORES]  # 查看SortSet中的元素,默认升序,加上WITHSCORES会打印分数
	redis> ZRANGE key 0 -1 withscores
redis> ZREVRANGE key start stop [WITHSCORES]  # 查看SortSet中的元素,降序
	redis> ZREVRANGE key 0 -1 
	
# HASH
redis> HSET key [key1 value]
redis> HGET key key1

redis> HMSET key [key1 value]...
redis> HMGET key [key1 key2 ...]
...
>
> API 接口文档: [http://redis.cn/clients.html](http://redis.cn/clients.html)
...
redisContext *redisConnectWithTimeout(const char *ip, int port, const struct timeval tv); // 超时连接
